...
const Home = ({ t, router }) => {
  ...
  const [csset, setcsset] = useState(0);
  ...
  const orderBuy = async () => {
    ...
    for (var i = 0; i < bAmount; i++) {
      const orderPrice = (
        bHighPrice * 1 -
        i * spreads -
        spreads * ((Math.random() * 50 + 50) / 100)
      ).toFixed(2);
      orders.push({
        volume: (
          parseFloat(bAsset) *
          ((Math.random() * 70 + 50) / 100)
        ).toFixed(5),
        price: orderPrice,
        direction: 'BID',
        symbol: symbol,
  ...
  const orderSell = async () => {
    ...
    for (var i = 0; i < sAmount; i++) {
      ...
      orders.push({
        volume: (parseFloat(csset) * ((Math.random() * 70 + 50) / 100)).toFixed(
          5
        ),
        price: orderPrice,
        direction: 'ASK',
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    <input
                      type="number"
                      placeholder="输入每个格子数量(价格不能低于10U)"
                      value={bAsset}
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    <input
                      type="number"
                      placeholder="输入每个格子出售数量(价格不能低于10U)"
                      value={csset}
                      onChange={(e) => setcsset(e.target.value)}
                    />
                  </td>
                  <td>
                    <input
